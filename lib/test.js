...
const fsStreamEnd = require('./_close-or-error')
...
build.pdf(argv.mdSource, argv.cssSource, (error, pdf) => {
  ...
  runSeries([
    ...
    (next) => {
      ...
      runParallel([
        (done) => fsStreamEnd(write, done),
        (done) => streamEnd(parse, done)
      ], next)
    }
  ], (error) => {
    assert.ifError(error)
      assert(errorLogs.length === 0, `expected no build errors but got ${errorLogs}`)
